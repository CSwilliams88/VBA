...
' keep track of current ticker
    Dim ticker As String
' keep track of number of tickers for each worksheet
    Dim number_tickers As Integer
' keep track of the last row in each worksheet.
    Dim lastRowState As Long
' keep track of opening price for specific year
    Dim opening_price As Double
' keep track of closing price for specific year
    Dim closing_price As Double
' keep track of yearly change
    Dim yearly_change As Double
' keep track of percent change
    Dim percent_change As Double
' keep track of total stock volume
    Dim total_stock_volume As Double
' keep track of greatest percent increase value for specific year.
    Dim greatest_percent_increase As Double
' keep track of the ticker that has the greatest percent increase.
    Dim greatest_percent_increase_ticker As String
' keep track of the greatest percent decrease value for specific year.
    Dim greatest_percent_decrease As Double
' keep track of the ticker that has the greatest percent decrease.
    Dim greatest_percent_decrease_ticker As String
' keep track of the greatest stock volume value for specific year.
    Dim greatest_stock_volume As Double
' keep track of the ticker that has the greatest stock volume.
...
' loop over each worksheet in the workbook
    ...
    ' Set total stock volume back to 0 when we get to a different ticker
        total_stock_volume = 0
    End If
